import { useNavigate } from "react-router"


const BlogCard = ({title, description, category, image, _id}) => {
    const navigate = useNavigate()
  return (
    <div onClick={() => navigate(`/blog/${_id}`)} className='w-full rounded-lg overflow-hidden shadow-sm hover:scale-102 hover:shadow-primary/25 duration-300 cursor-pointer'>
      <img src={image} alt={title} className='w-full' />
      <span className='ml-5 mt-4 px-3 py-1 inline-block bg-primary/20 rounded-full text-primary text-xs'>{category}</span>
      <div className='p-5'>
        <h2 className='mb-2 font-medium text-gray-900'>{title}</h2>
        <p className='mb-3 text-xs text-gray-600' dangerouslySetInnerHTML={{__html: description.slice(0,100) + '...'}}></p>
      </div>
    </div>
  )
}

export default BlogCard
